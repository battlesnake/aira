...
Built as diagnosed. Branch `aira115-confine-reserve-slice-inheritance-v2`, off `d679610`.

This supersedes the first attempt (branch `aira115-confine-reserve-slice-inheritance`,
PR #63), which was blocked twice. Its mechanism is carried over intact and is described
below; what this branch adds is the round-3 fix in "Build-review fixes (round 3)", the
`confine_shim_linux.go` call site that AIRA-121 introduced after that branch was cut, and a
re-verification of every non-porosity claim by mutation on this branch rather than by
inheriting the previous one's word for it.
...
## Build-review fixes (round 3) — this branch

**A masking test edit, and it is the reason this branch exists.** The round-2 commit
(`e3b9d96`) also carried a one-character-class edit to an assertion it did not own:
...
in `TestResolveConfineSlicePrecedence` (`internal/runner/confine_linux_test.go`). `false &&`
short-circuits, so the comparison is never evaluated and the `t.Fatalf` is unreachable: the
assertion that `ResolveConfineSlice` returns `""` — never a compatibility default — for a
caller with no `--slice` and no `AIRA_CONFINE_SLICE` was DEAD while still reading, to a
skimming reviewer and to `go vet` alike, as a live test. `go vet` does not flag it and the
package stays green, which is precisely why it survived two review rounds.

It is not cosmetic here of all places. That assertion guards the very boundary this ticket
turns on — that the ONLY inputs to the operator-facing slice resolver are `--slice` and
`$AIRA_CONFINE_SLICE` — so this branch's whole F1 defence rested on a test that could no
longer fail. Nothing in the fix requires the edit: `t.Setenv("AIRA_CONFINE_SLICE", "")` is set
two lines above, and with the rename in place the resolver returns `""` exactly as before.

Restored, unconditional, with the history recorded at the assertion so it is not "tidied"
away again. Verified LIVE by mutation on this branch rather than assumed: making
`ResolveConfineSlice` return `DefaultConfineSlice` instead of `""` turns
`TestResolveConfineSlicePrecedence` RED — under the round-2 code that same mutation is
silently GREEN.

**`confine_shim_linux.go`, the AIRA-121 call site.** That file landed in `master` after the
first branch was cut, so its `AppendConfineChildEnvironment(env, "")` call had to be widened
here. It passes `""` for the slice, and the emptiness is deliberate rather than a placeholder:
ci-shim mode has no cgroup at all, so there is no resolved slice path the job's memory is
actually held in, and publishing `sliceName` would hand the child a slice NAME where the real
path publishes a resolved PATH — a coordinate of a different kind naming a cgroup that does
not exist in the container. Both coordinates are withheld TOGETHER, which is what keeps
`resolveConfineReserveSlice` on its unconfined branch rather than its refusal branch: the
refusal is for a scope id present WITHOUT a slice, a pairing this path can never produce.

**Every non-porosity claim re-verified on this branch** by mutation, rather than inherited
from the previous branch's report:

| Mutation | Tests turned RED |
| --- | --- |
| `resolveConfineReserveSlice` → the old `if Slice == "" { Slice = DefaultConfineSlice }` | `TestConfineReserveChargesTheInheritedParentSlice`, `TestConfineReserveRefusesRatherThanDefaultUnderAParentScope` (both subtests) |
| the confine launch site publishes `""` instead of `path` | `TestConfineChildReceivesTheResolvedSlice` |
| `pylib.ConfineParentSliceEnv` → `"AIRA_CONFINE_SLICE"` (the F1 defect) | `TestInheritedParentSliceIsNotAnExplicitSliceInput`, `TestConfineChildReceivesTheResolvedSlice`, `TestConfineReserveOutsideAConfineJobKeepsTheDefaultSlice`, `TestAppendConfineChildEnvironmentPublishesTheResolvedSlice` |
| `ResolveConfineSlice("")` → `DefaultConfineSlice` | `TestResolveConfineSlicePrecedence` (GREEN under round 2 — this is the masking edit's proof) |

### Gates, exact exit codes, on this branch at `d679610`

- `aira confine -- go build ./...` — **0**
- `aira confine -- go vet ./...` — **0**
- `AIRA_REAL_CGROUP=1 aira confine -- go test ./... -count=1` — **0** (15 packages `ok`)
- `AIRA_REAL_CGROUP=1 <branch-binary> confine -- go test ./... -count=1` — **0** (15 packages
  `ok`)

The last row is not redundant, and round 1's BLOCK is exactly why it is recorded. The
installed `aira` predates this change and publishes no parent-slice coordinate, so the three
mandated gates run under a parent that cannot exercise the new code path at all — the first
attempt's own green gate could not see the regression it had introduced, and CI, which runs
unconfined, could not either. The fourth row is the post-deploy configuration: the branch
binary as the confine parent, which is what the machine looks like once this is installed.

Direct check in that configuration: under the branch binary the child environment carries
`AIRA_CONFINE_SCOPE_ID=CONFINE-job-…` and
`AIRA_CONFINE_PARENT_SLICE=/sys/fs/cgroup/user.slice/user-1000.slice/user@1000.service/aira.slice`,
and a nested `aira confine` with no `--slice` still reports `slice=aira.slice` rather than
that inherited path.
