...
- **Wiring.** A run `--phase work-review --tool codex` auto-emits a ComputeEvent (parse `--json` usage; absent → wall/cpu/peak-RSS, buckets `unevaluated`); a `--report`ing run creates a §13 report and attests `tests-green` **only** per the exit==0-and-test-count>0 rule (§9).
