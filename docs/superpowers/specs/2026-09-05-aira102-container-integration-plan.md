...
**Boundary proof (new in v2 — closes Sol P0-3 / Fable MF1).** Both reviewers produced argv where the v1 rule *acted wrongly* rather than declining: `docker run --rm qemu-image qemu-system-x86_64 -m 4G` (qemu's own RAM flag) and `docker run alpine echo --memory=8g` would `Establish` a limit that does not exist, raise the ledger for a phantom, print `caller=4G` as a fact, and fire §3.6's loud line on a meaningless comparison. So: **stop scanning at the first token that is neither `-`-prefixed nor immediately preceded by a `-`-prefixed token** — the earliest position at which the image can appear. A memory-shaped token at or after that point is **ignored entirely** (neither `Present` nor `Established`), because the boundary proof establishes it belongs to the container's own command. *(v2 made such a token `Present` + `unevaluated:after-image-candidate`; the round-2 correction noted above replaced that — the over-inclusion bought no safety and defeated this ticket's own target case.)*
