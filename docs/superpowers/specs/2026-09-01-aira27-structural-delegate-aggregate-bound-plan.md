...
## 8. Implementation threading notes for the build (from the Fable re-gate; design is FOLDS-SOUND)

These are build-brief threading requirements, not open design points — each is loud-failing or pinned by a §5 test:

- **§3.2 discriminator + citation.** cpu.weight aging is CLIENT-side (`confine_linux.go:550-555`), not `governor.go:289-368` — the RAM-ordering inerting touches only the daemon governor. Make ordering inert *per worker* keyed on the `@drc-` marker via `governor jobID == AIRA_CONFINE_SCOPE_ID` (`cmd/aira/main.go:895` → `governor.go:695`), so §3.2 and §3.6 compose. The whole RAM-ordering mechanism hangs off `ramOrdering.enabled` (`governor.go:393-415`) — the sort tiebreak (`:436-437`), skip/park loop (`:449-471`), force valve (`:475-482`), availability decrement (`:505-509`), displacement credit (`:557-570`) all go vacuous together; with it inert, selection degrades cleanly to the pre-Slice-3 age/jobID/seq fill (`:428-443`) — no capacity/fairness dependency. `heldRSS`/`nextEst` have no other consumers.
- **§3.6 — the `@dr-` literal is in FOUR places, thread all four:** the exact-prefix matcher (`confine_manage_linux.go:269-271`), the **parse strip (`:242-243`)** — MISS THIS and `@drc-` scopes are invisible to `ListConfines` → adopted at **0** (worse than the bug; §5 test 5 pins it), the daemon scope-id regex (`admit.go:110`), and the name extractor (`admit.go:959`) — miss either of the last two and every `@drc-` admission fails loudly `E_PROTOCOL` (`admit.go:940-941`). Verified clean: `@drc-` does not match the `@dr-` exact-prefix test ('c' ≠ '-'), so full-cap adoption (`admit.go:686`) is reached with no loop change, and full-cap adoption is not a double-count (held-set exclusion `:640-645,658-660` + `checkedAvailable` `max(...)` not additive `:743-760`).
- **§3.6 disconnect-hold placement:** the hold must live in the **adopted/pending** ledger, NOT the connection-held set (else it blocks the #72 reaper's no-live-lease check). It is bounded: orderly teardown precedes EOF (LIFO defer `confine_linux.go:525` before `:549`), so the hold only matters on disorderly death, and subtree-aware liveness (`cgroup.events populated` → 0 when the last pid dies, the #72 reaper mechanism) guarantees a genuinely-dead scope cannot hold its charge forever.
- **§3.7 — remove the fallback for DelegateRAM entirely, don't gate on "daemon unavailable":** the flock fallback is reached via dial failure AND every protocol fall-through (`admission_linux.go:355,359,391,395` → `:129-141`), which is broader than "unavailable". Implement as `confine-reserve`'s shape — **no flock fallback path at all for DelegateRAM** (`confine_reserve.go:15-16`); §5 test 3 pins it. Non-delegate flock fallback is untouched. Refusal is deadlock-free (admission at `confine_linux.go:509`, error returns at `:520-521` before scope creation `:535` — the refused job holds no lease/lock/worker).
