...
> **GATE-3 FOLD (2026-09-12) — BUILD-CLEARED.** GATE-3: every GATE-2 item closes on the code; the load-bearing mint-pid==monitor-pid identity holds (daemon copies the pid out of `parent_scope_id`). Two one-line corrections folded (spec §16.2): (a) escape attestation's honest verdict is **`unverified` with no escape**, NOT `contained` (leader-only by the #20 design) — exemption in the single `witnessedEscape` chokepoint; builder must NOT weaken #20 to force `contained` (Task 5, Gate E); (b) the `serveReDeclare` kill hook is **anchor-gated** (fire only when the anchored release discharged) to survive the late-ack redial race (Task 4). The gate's verdict: **buildable, no fourth round warranted.** The next quality gate is the build-review, not another plan-gate.
...
- [ ] **Step 4: Run, confirm pass**; verify the invariants `isSubReservation` actually protects (CORRECTED per the T1/T2 build-review — workers DO count in `outstandingJobs`; the flag gates the **exclusivity-gate exemption** `admit.go:552` and **oomsteer child aggregation** `oomsteer.go:445`, NOT the job count), i.e. a worker with a non-empty `parent_scope_id` stays exempt from the exclusivity gate and out of the parent's oomsteer child sum. **The real-cgroup e2e harness must publish a canonical `AIRA_CONFINE_SCOPE_ID` as of THIS task** (not Task 9) — otherwise every existing real-cgroup aitest e2e reds on refuse-empty (P3c).
- [ ] **Step 5: Commit.**
...
> **BUILD-REVIEW ADDITIONS (from the T3+T6 gate — fold into this task):**
> - **P2-6 (un-mask the restart gate):** post-T3 the §16.1 re-anchor assertions at `restart_merge_gate_worker_e2e_test.go:397/:569` are **masked** by an earlier `mkdir` failure at `:333/:521` (the fixture's `restartGateFixtureSlice` is a fake path workers now real-mkdir under). T4 must (a) fix the fixture — point `startServer`'s `SetAdmitResolveSliceForTest` at the real `h.parent` (as T3 did for the other harness) — AND (b) fix the dirname re-declare key, AND (c) **see :397/:569 GREEN**, not merely make the mkdir succeed. Making the fixture pass without the re-key would be a false pass.
> - **P2-1 (delegation hardening):** add an idempotent `ensureConfineDelegation(parent)` before `backend.Create` in `CreateWorkerScope` (`worker_scope_linux.go`). Without it, if the slice's `subtree_control` loses `+memory` (window opens with T7's outer-scope change / a systemd reset), the worker `memory.max` write ENOENTs → `request-invalid` terminal → whole suite `unevaluated` (fail-closed, no self-heal). One idempotent call closes it.
> - **P2-2 (cpu.weight):** worker sibling scopes get no `cpu.weight`/aging (the ordinary confine path sets 100→10 over 30 min, `confine_linux.go:808-814`). Nested workers shared the outer's single weight; N siblings now hold N+1 shares against other confine jobs under contention. Write a weight on worker scopes (or explicitly record the gap in §13 — prefer writing it).
...
- (ii) `serveReDeclare` installs the **same** peer-EOF kill+rmdir when `scopeID != "" && parentScopeID != ""` (that shape is uniquely a worker lease — a scoped *ordinary* admit carries no `parent_scope_id`, admit.go:3027-3030), so a post-restart parent-kill does not re-orphan mid-test workers. **Anchor-gated (§16.2, P1-B):** fire the kill+rmdir **only when the anchored release actually discharged** — `releaseAdmitWaiterAnchored` (admit.go:2564) already returns that bool. A late ack (> the 5 s `leaseKeeperExchangeGrace`) redials: conn1 closes while conn2 re-anchors the **live** worker; a bare-`peerCtx.Done()` kill would `cgroup.kill` a mid-test worker (a release is idempotent; a kill is not). **Accepted gap:** the reverse order (conn1 EOF processed before conn2's frame → a legitimate discharge → kill) re-runs that one test via the supervisor's crash/retry path, bounded by 5 s × `maxNoAck`(=10).

- [ ] **Step 1: Failing tests** — (unit) on peer-EOF the worker scope is killed+removed; on `s.stopping` it is NOT. (unit/merge-gate) after a simulated restart + re-declare, the lease key is the dirname (not a path) AND a subsequent peer-EOF kills+rmdirs the re-declared worker. **(unit, P1-B) re-anchor on conn2, then EOF conn1 → worker NOT killed** (kill only fires when the anchored release discharged). (e2e, Task 9 Gate D) kill the parent → no worker survives, no dir remains — both fresh and post-restart. Expected: FAIL (today relay-EOF frees the lease but leaves process+dir; re-declare keys by path and installs no kill).
- [ ] **Step 2: Run, confirm fail.**
...
## Task 5: Exempt the worker fork→place_self migration from escape attestation — LOCAL pid check (P1-2, P1-B, §16c/§16.1/§16.2)

**Files:** `internal/runner/runner_linux.go` **`witnessedEscape` only** (the single chokepoint backing the sampler :1811, the teardown :1946, and the classifier's escape check :1593 — it already holds `observation.Cgroup`), `confine_linux.go` :1237,1400-1408; Test: Task 9 Gate E + mutation.

**Interfaces:** A process seen alive in a sibling cgroup whose basename parses (`parseConfineScopeID`) to **name prefix `aitest-w` AND embedded pid == `os.Getpid()`** (this monitor's own pid — which is the pid Task 1 minted into the worker name) is NOT a witnessed escape. Purely local positive check — no lease table, no daemon round-trip — and it holds **through teardown** (on `--timeout`/`--kill` the relays die and leases release, but the pid-in-the-name is still `os.Getpid()`, so the killed-run case no longer false-flags an escape — the GATE-2 P1-B defect). A genuine escape to any *other* cgroup (no `aitest-w` name / wrong pid) stays witnessed.

> **GATE-3 correction (§16.2):** the honest verdict is **`unverified`**, NOT `contained`. `contained` is leader-only by the #20 attestation design; a pytest supervisor always has relay descendants → `HadDescendants` → `ScopeUnverified` by design. The exemption removes only the **escape** verdict (`descendant-escaped`/`migrated`), which is evaluated *before* the `HadDescendants` rule. **Builder hazard — DO NOT weaken the #20 leader-only rule to force `contained`; the gate asserts `unverified`-without-escape, and that is correct.**

- [ ] **Step 1: Failing gate** (Task 9 Gate E) — a delegate run attests `scope-integrity=unverified` **with no `descendant_escape`** (NOT `descendant-escaped`/`migrated`), including on the `--kill`/`--timeout` teardown path. Expected: FAIL (every worker migration is currently witnessed → `descendant-escaped`).
- [ ] **Step 2: Run, confirm fail.**
- [ ] **Step 3: Implement** the local exemption in the single `witnessedEscape` chokepoint. Requires Task 1's parent-pid slot and Task 2's parseability guarantee.
- [ ] **Step 4: Run, confirm pass**; confirm a genuine escape (a process in an unrelated cgroup, or an `aitest-w` name whose pid ≠ this monitor) is **still** witnessed (don't over-exempt); mutation: drop the pid check → Gate E sees the escape verdict return → reds.
- [ ] **Step 5: Commit.**
...
- [ ] **Gate E — `...ScopeIntegrityNoEscape` (P1-2, §16.2):** a delegate run attests `scope-integrity=unverified` with **no `descendant_escape`** — NOT `descendant-escaped`/`migrated` (and NOT `contained`, which is leader-only by design). Mutation: drop the exemption → the escape verdict returns → reds.
...
**Subagent-Driven** (fresh Opus builder per task-group + Fable build-review), per the two-loop. Build order: T1 → T2 → T3+T6 (coupled) → T4 → T5 → T7 → T8 → T9 → T10 (T1's pid slot feeds T5; T8's `scope_path` feeds T9 Gate A). **Plan-gate cleared at GATE-3** (GATE-1 four nesting-jobs → GATE-2 two restart gaps → GATE-3 two one-line fixes → build-cleared). The next quality gate is the **build-review** on the actual code, not another plan-gate. **Queued follow-up:** the owner's "reservation requested = reservation given" simplification is a SEPARATE slice (deletes the history→grant estimation subsystem globally; removes Task 7's parent-signature namespacing as trivial cleanup) — pending the owner's scope nod; does not block this slice.
