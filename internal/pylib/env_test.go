...
func TestAppendAitestChildEnvironmentInjectsAndStripsStaleKeys(t *testing.T) {
	...
	if got["AIRA_AITEST_WORKER_ADMIT_CMD"] != "/opt/aira" {
		t.Fatalf("worker admit cmd=%v", got)
	}
	// The stale inherited grade is replaced by the launch's own (S2a).
	if got["AIRA_AITEST_ADMISSION"] != "cgroup-sub-scope" {
		t.Fatalf("AIRA_AITEST_ADMISSION=%q", got["AIRA_AITEST_ADMISSION"])
	}
...
// "unset" that disables daemon-backed admission.
//
// verifies: AIRA-44
