...
func renderMarkdownBody(title string, actions []SkillAction, contract ResponseContractSpec) string {
	...
	out.WriteString("Any memory-heavy shell command — test suites, builds, data loads, sims/training/sweeps, dev servers, long-running jobs — MUST be run under `aira confine -- <cmd>`. It runs the job in a machine-wide memory-capped slice with `memory.oom.group` group-kill, deprioritises it (nice/ionice/oom_score_adj), and RAM-gates admission, so a runaway dies in-slice instead of OOM-killing the machine. A plain `aira confine -- <cmd>` reserves RAM for the WHOLE job (from that command's peak-RSS history, or a sane default when it has none) — right for builds and one-shot commands, but it over-reserves a large test suite and needlessly contends the shared cap. For a pytest suite, prefer `aira confine --delegate-ram --memory-reserve <framework> -- pytest ...`: the governor then admits and RAM-caps each test as it runs (blocking and gc-collecting workers to stay under the cap) instead of holding one whole-suite reservation. This works through a make/wrapper target too — `aira confine --delegate-ram --memory-reserve <framework> -- make <gate>` governs the pytest legs it spawns (the plugin environment is inherited by descendant pytest processes); such a job admits on a small framework overhead and then throttles PER TEST (a test whose reservation does not fit waits while sibling workers keep running), so a suite is never blocked at the door behind one large whole-job reservation, and non-pytest legs in a mixed target (lint, type-check, build) are not per-test governed and fall back to the slice cap + `memory.oom.group`. Know the trade on a CONTENDED shared box: `--delegate-ram` is efficient (per-test) but NOT airtight — a delegate-ram scope's `memory.max` is a generous ceiling (not its reserve) and the per-test gate fails OPEN under contention, so under heavy multi-suite load a delegate suite can grow past what admission accounts, over-commit the slice, and a slice-level OOM may kill a well-behaved NEIGHBOUR. A plain non-delegate whole-job `aira confine --memory-reserve R -- <cmd>` is airtight by contrast — the scope is hard-capped at R (a runaway self-OOMs, never over-commits a neighbour) — at the cost of reserving the whole-job peak (coarser, may wait under contention). Note `--memory-max N` on a non-delegate job UP-CHARGES the admission reserve to N (it does NOT let you 'cap high, reserve low' — that combination over-reserves, it never under-reserves). You only DECLARE a RAM estimate where it matters — put an `aira_mem(\"4G\")` marker on the heavy outlier tests (sizes are 1024-based; decimal K/M/G/T with optional i and/or B, e.g. `aira_mem(\"512MB\")`, `aira_mem(\"1.5GB\")`, or `aira_mem(\"4GiB\")`), and pass `--memory-reserve` for the suite framework; every unannotated test falls back to a sane default per-test estimate, so the MAJORITY of tests need no annotation. This applies in EVERY working directory: `aira confine` is project-less and needs no `.aira/config`. For long-lived agent sessions, `export AIRA_CONFINE_OWNER=<stable-session-id>` before launching or managing jobs. To stop a confined job, first run `aira confine --list`, then run `aira confine --kill <name|supervisor-pid|scope-id>`; use `--steal` only when intentionally overriding unknown or foreign ownership. Kill the scope, not a bash wrapper or a PID picked by eye. Never `kill -9` the supervisor, because that can orphan its child in the still-capped scope, and never `systemctl --user stop` the shared slice. On a contended shared machine, admission itself can legitimately queue for many minutes (tens of minutes under real load) before the job even starts — a foreground invocation with a short or default tool timeout can be killed mid-queue by the CALLER, not by AIRA, wasting the whole wait; prefer running a confine invocation in the background (or with a generous explicit timeout) over assuming a short default is safe. `aira confine` has no native `--detach` of its own yet (it is a plain foreground process, tied to its caller's lifetime) — `aira run --detach` exists for general subprocess survival, but for RAM-confined jobs specifically, backgrounding via the calling harness is the current workaround, not a guarantee: background-job status tracking has its own known, unresolved reliability reports independent of AIRA, so treat it as risk reduction, not a promise. Trivial read-only commands (`ls`, `cat`, `grep`, `git status/log/diff`) run unconfined.\n\n")
	out.WriteString("## Running pytest suites with aitest\n\n")
	out.WriteString("For an AIRA-governed Python project, prefer `aitest` over `pytest-xdist` for parallel test execution: activate with `--aitest-workers=N` or `--aitest-workers=auto` (up to the host's CPU count) instead of xdist's `-n`. aitest has TWO independent preconditions and does nothing useful unless BOTH are met. FIRST, the project must register the plugin itself: `aira confine` exports `AIRA_AITEST_LIB` but sets no `PYTHONPATH`, so add a GUARDED snippet to the project's `conftest.py` — read `AIRA_AITEST_LIB` and, only if it is set, insert it into `sys.path`, `importlib.import_module('aitest')`, then set `pytest_plugins = ('aitest',)`. The guard is load-bearing: an unguarded import breaks every plain `pytest` or CI run that has no AIRA in the environment. Without this step `--aitest-workers` is not a recognised pytest option at all. SECOND, launch under `aira confine --delegate-ram -- pytest --aitest-workers=auto ...`. `--delegate-ram` is REQUIRED for aitest and is the only flag you must add — you still need no `--memory-reserve` (a delegate job pins a small 512M framework overhead by itself) and no per-test `aira_mem()` markers. It is required because aitest's supervisor forks each worker and admits it individually through the daemon (`worker-admit`), which places that worker in its own kernel-enforced cgroup sub-scope NESTED UNDER this job's own outer scope, sized from a per-worker backstop (override via `AIRA_AITEST_ESTIMATED_BYTES` only for an unusually memory-hungry suite); `worker-admit` refuses to grant anything unless that outer scope has a finite `memory.max`, and `--delegate-ram` is the only launch shape that BOTH wires these coordinates at all AND is guaranteed such a cap on every path — a non-delegate job can also be finite, via `--memory-max` or a declared `--memory-reserve`, but it never receives the coordinates, and its reserve is deliberately left uncapped when unpinned. Launched WITHOUT `--delegate-ram` the `AIRA_AITEST_*` coordinates are stripped from the child environment by design, so the suite either fails outright with an unrecognised-argument error (when registered as above) or degrades to ONE uncontained worker after a single easy-to-miss stderr warning — never the per-worker containment you asked for. Treat any existing plain `aira confine -- pytest --aitest-workers=...` in a script or Makefile as broken, not as a working alternative. Know exactly what this does and does not account for: each worker's sub-scope cap is real and kernel-enforced, but worker admission is decided against THIS JOB's own outer ceiling, not against the shared machine-wide slice ledger: `worker-admit` adds no slice-ledger charge of its own, and a normally-admitted delegate job contributes just its 512M framework overhead to the slice. So the `NOT airtight` caveat above applies to aitest with MORE force than to an ordinary delegate-ram suite: workers can grow into the outer ceiling without the slice accounting for it at all. If the daemon is genuinely unreachable, aitest still runs: it falls back to its own reduced worker pool (`n_workers <= min(requested, NumCPU)`) with no cgroup placement and a visible warning — it never falls back to xdist or serial pytest. One migration hazard for a project still moving off xdist: `--delegate-ram` also sets `AIRA_TEST_MEM_GOVERNOR=1`, which arms the LEGACY `aira_xdist_governor` plugin's per-test reservations inside every aitest worker if that plugin is also registered, and `AIRA_GOVERNOR=off` does NOT disable that half (it only turns off the governor's CPU checkpoint). A project adopting aitest should stop registering `aira_xdist_governor` in its conftest, or block it per invocation with `-p no:aira_xdist_governor`. `--aitest-workers` is a new, explicit flag, not a reinterpretation of xdist's `-n`, so a project that doesn't pass it runs exactly as it always did (xdist if separately configured, or serial otherwise) — opting in is purely additive.\n\n")
