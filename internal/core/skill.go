...
func renderMarkdownBody(title string, actions []SkillAction, contract ResponseContractSpec) string {
	...
	out.WriteString("For an AIRA-governed Python project, prefer `aitest` over `pytest-xdist` for parallel test execution: activate with `--aitest-workers=N` or `--aitest-workers=auto` (up to the host's CPU count) instead of xdist's `-n`. Launch the whole invocation under a plain `aira confine -- pytest --aitest-workers=auto ...` — no `--delegate-ram`, no `--memory-reserve`, no per-test `aira_mem()` markers. aitest's own supervisor forks each worker and admits it individually through the daemon (`worker-admit`), which places it in its own kernel-enforced cgroup sub-scope sized from a per-worker backstop (override via `AIRA_AITEST_ESTIMATED_BYTES` only for an unusually memory-hungry suite) — so worker count and per-worker RAM containment are handled automatically, not something you hand-tune per suite the way the old xdist-governor pattern required. If the daemon is unavailable, aitest still runs: it falls back to its own reduced worker pool (`n_workers <= min(requested, NumCPU)`) with no cgroup placement and a visible warning — it never falls back to xdist or serial pytest. `--aitest-workers` is a new, explicit flag, not a reinterpretation of xdist's `-n`, so a project that doesn't pass it runs exactly as it always did (xdist if separately configured, or serial otherwise) — opting in is purely additive.\n\n")
