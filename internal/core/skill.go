...
func renderMarkdownBody(title string, actions []SkillAction, contract ResponseContractSpec) string {
	...
	out.WriteString("Any memory-heavy shell command — test suites, builds, data loads, sims/training/sweeps, dev servers, long-running jobs — MUST be run under `aira confine -- <cmd>`. It runs the job in a machine-wide memory-capped slice with `memory.oom.group` group-kill, deprioritises it (nice/ionice/oom_score_adj), and RAM-gates admission, so a runaway dies in-slice instead of OOM-killing the machine. This applies in EVERY working directory: `aira confine` is project-less and needs no `.aira/config`. For long-lived agent sessions, `export AIRA_CONFINE_OWNER=<stable-session-id>` before launching or managing jobs. To stop a confined job, first run `aira confine --list`, then run `aira confine --kill <name|supervisor-pid|scope-id>`; use `--steal` only when intentionally overriding unknown or foreign ownership. Kill the scope, not a bash wrapper or a PID picked by eye. Never `kill -9` the supervisor, because that can orphan its child in the still-capped scope, and never `systemctl --user stop` the shared slice. Trivial read-only commands (`ls`, `cat`, `grep`, `git status/log/diff`) run unconfined.\n\n")
