...
func TestExternalCgroupKillProducesNoOOMAdvisory(t *testing.T) {
	...
	advisory := formatConfineReserveAdvisory(scopeMemoryMax, usage.PeakRSS, oom, ConfineCapSourceMemoryMax, 64<<30)
	...
	oomCount := int64(1)
	if reported := formatConfineReserveAdvisory(scopeMemoryMax, usage.PeakRSS, oomCount > 0, ConfineCapSourceMemoryMax, 64<<30); !strings.Contains(reported, "OOM-killed") {
