...
// outside this package -- cmd/aira's confine launch branches on it (real vs
// ci-shim), and a test of that branch has no other way to install a mode.
// Production resolves its mode once, because the record is written at
// image-build time and cannot change under a running job.
